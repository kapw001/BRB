...
//                in.putExtra("unlockscreen", true);
                                ...
                                final String contactName = getContactName(number, context);
                                message = contactName + ",\n" + message;
                                ...
                                try {
                                    if (sharedPreferences.getInt("radio", R.id.whatsappRadio) == R.id.whatsappRadio) {
                                        ...
                                        new Handler().postDelayed(new Runnable() {
                                            @Override
                                            public void run() {
                                                openWhatsApp(context, finalContactNum + "@s.whatsapp.net", contactName, finalContactNum);
                                            }
                                        }, 1000);
                                        ...
                                        if (sharedPreferences.getBoolean("driving", false)) {
                                            ...
                                            sendSMS(context, message, contactNum, contactName);

                                        } else {

                                            final String finalContactNum1 = contactNum;
                                            new Handler().postDelayed(new Runnable() {
                                                @Override
                                                public void run() {
                                                    if (!contactName.contentEquals("")) {
                                                        openSms(context, message, finalContactNum1, contactName);
                                                    }
                                                }
                                            }, 1000);
...
//                                    Log.e("Call reciever", e.getMessage());
    ...
    private String openWhatsApp(Context ctx, String id, String contactName, String contactNum) {
...
//        Toast.makeText(ctx, "" + isValidLocalSMSSend(number), Toast.LENGTH_SHORT).show();
    ...
    private void sendSMS(Context ctx, String message, String number, String contactName) {
